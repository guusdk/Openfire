...
public class LocalOutgoingServerSession extends LocalServerSession implements OutgoingServerSession {
    ...
    public static LocalOutgoingServerSession createSession(String serverName, XmlPullParser xpp, Connection connection, boolean directTLS) throws XmlPullParserException, IOException
    {
        final Logger log = LoggerFactory.getLogger( Log.getName() + "[Creating session based on connection: '" + connection + "'']" );
        ...
        final String namespace = xpp.getNamespace(xpp.getPrefix());
        if (!namespace.equals(ETHERX_NAMESPACE) ) {
            throw new XmlPullParserException(LocaleUtils.getLocalizedString("admin.error.bad-namespace") + ": " + namespace);
        }
//            if (directTLS) {
//                try {
...
//        }
        ...
        throw new IOException("Unable to create an outgoing server session");
    }
    ...
    private static LocalOutgoingServerSession createOutgoingSession(String localDomain, String remoteDomain, int port) {
        ...
        try {
            ...
            socketConnector.getFilterChain().addAfter( ConnectionManagerImpl.EXECUTOR_FILTER_NAME, 
                ConnectionManagerImpl.XMPP_CODEC_FILTER_NAME, new ProtocolCodecFilter( new XMPPCodecFactory() ) );
...
//
            ...
            final CompletableFuture<Boolean> successCallback = new CompletableFuture<>();
            socketConnector.setHandler(new OutgoingServerConnectionHandler(getConnectionConfiguration(), localDomain, remoteDomain, port, successCallback));
    ...
    private static ConnectionConfiguration getConnectionConfiguration()  {
        try {
            return new ConnectionConfiguration(
                ...
                XMPPServer.getInstance().getCertificateStoreManager().getIdentityStoreConfiguration(ConnectionType.SOCKET_S2S),
...
//      TODO      xpp.setInput(new InputStreamReader(connection.getTLSStreamHandler().getInputStream(), StandardCharsets.UTF_8));
    ...
    private static LocalOutgoingServerSession attemptSASLexternal(Connection connection, MXParser xpp, XMPPPacketReader reader, String localDomain, String remoteDomain, String id, StringBuilder openingStream) throws DocumentException, IOException, XmlPullParserException {
...
//  TODO           xpp.setInput(new InputStreamReader(connection.getTLSStreamHandler().getInputStream(), StandardCharsets.UTF_8));
    ...
    private static boolean doExternalAuthentication(String localDomain, Connection connection,
